...
public class JsfPaginationBean {
	...
	private List<Subscription> cdList;
	...
	public JsfPaginationBean() {
		...
		rowsPerPage = 2; // Default rows per page (max amount of rows to be displayed at once).
		pageRange = 2; // Default page range (max amount of page links to be displayed at once).
	}
...
//		return cdList;
//	}
	public List<Subscription> getSubscriptionList(String uhid) throws ParseException{
		//public String getSubscriptionList(String uhid) throws ParseException{
		Map<String, Object> sessionMap = FacesContext.getCurrentInstance().getExternalContext().getSessionMap();
		sessionMap.put("uhid", uhid);
		System.out.println("List of Insurance of this UHID is : " +uhid);
		if (cdList == null) {
			loadSubscription(uhid);
		}
	...
	private void loadSubscription(String uhid) throws ParseException{
		System.out.println("First Row  " +firstRow);
		System.out.println("Count  " +rowsPerPage);
		
		cdList = queryHelper.getListOfSubscription(uhid, firstRow, rowsPerPage);
		//cdList = queryHelper.getListOfSubscription(firstRow, rowsPerPage);
		
		System.out.println("Subscription Count is  " +cdList);
		totalRows = queryHelper.countRows(uhid);
